...
def run_assessment_feature_snapshot_build_job(*, session, job) -> None:
    build_assessment_artifacts_for_run(
        session=session,
...
def run_comparable_retrieval_build_job(*, session, job) -> None:
    build_assessment_artifacts_for_run(
        session=session,
        assessment_run_id=UUID(str(job.payload_json["assessment_id"])),
        requested_by=job.requested_by or "worker",
...
def run_replay_verification_batch_job(*, session, job) -> None:
    summary = replay_verify_all_assessments(session=session)
